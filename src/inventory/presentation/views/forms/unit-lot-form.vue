...
const unitLotId          = computed(() => editId.value);
...
const isValid = computed(() =>
  !!form.value.productId && form.value.quantity != null && !!form.value.entryDate
);
...
const selectedProduct = computed(() =>
  form.value.productId ? store.getUnitProductById(form.value.productId) : null
);
...
onMounted(() => {
  ...
  if (isEdit.value) {
    const lot = store.getUnitLotById(editId.value);
    if (lot) populate(lot);
  } else {
    form.value.codeQR    = `ULOT-${Date.now()}`;
...
watch(() => store.unit_lotsLoaded, (loaded) => {
...
function closeModal() {
  ...
  // Opened from unit-lot-list → go back to it; opened directly from lot-list → go to lot-list
  if (route.matched.some(r => r.name === 'inventory-unit-lots')) {
    router.push({ name: 'inventory-unit-lots', query: { productId: pId } });
  } else {
...
async function submit() {
  ...
  if (isEdit.value) await store.updateUnitLot({ ...payload, id: editId.value });
  else              await store.addUnitLot(payload);
  ...
  <div class="modal-overlay" @click.self="closeModal">
    <div class="modal-card">
      ...
      <form @submit.prevent="submit" class="custom-form" novalidate>
        ...
        <template v-if="isEdit">
          <div class="form-field-wrapper">
            ...
            <div class="readonly-container">
              <input type="text" :value="'lot-' + unitLotId" readonly class="grey-input">
        ...
        <!-- Quantity -->
        <div class="form-field-wrapper">
          <label class="label-bold">{{ t('lots.form.quantity') }} ({{ t('products.col.byUnit') }})</label>
...
.modal-card {
  ...
  /* no scrollbar — layout is compact enough */
