...
const form = ref({ name: '', description: '', codeQR: '', price: null, weightGrams: null, brand: '' });

const isValid = computed(() =>
  !!form.value.name && !!form.value.codeQR && form.value.price != null
);
...
onMounted(() => {
  ...
  if (isEdit.value) {
    const p = store.getUnitProductById(editId.value);
    if (p) populate(p);
  } else {
    form.value.codeQR = `UPROD-${Date.now()}`;
  }
});

watch(() => store.unit_productsLoaded, (loaded) => {
    ...
    <div class="modal">
      ...
      <form @submit.prevent="save" novalidate>
        ...
        <div class="field-group">
          ...
          <input class="field-input field-input--locked" :value="t('products.form.byUnit')" readonly />
        </div>
        ...
        <div class="field-group">
          ...
          <input class="field-input" v-model.number="form.weightGrams"
        ...
        <div class="field-group">
          ...
          <input class="field-input" v-model="form.brand" type="text"
