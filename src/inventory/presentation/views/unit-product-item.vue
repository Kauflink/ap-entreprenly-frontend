...
<template>
  <div class="row">
    ...
    <div class="center">${{ Number(product.price ?? 0).toFixed(2) }}</div>
    <div class="center">{{ product.weightGrams }}g</div>
    <div class="col-qr">
      <div class="qr-box">
...
<style scoped>
.row { display: grid; grid-template-columns: 1fr 1fr 120px 90px 80px 80px 90px; align-items: center; padding: 12px 20px; border-bottom: 1px solid var(--color-card-border); transition: background 0.15s; }
...
.brand { font-size: 13px; color: var(--color-text-secondary); }
