...
const selectedProduct = computed(() =>
  productId.value ? store.getUnitProductById(productId.value) : null
);

const filteredLots = computed(() =>
  productId.value
    ? store.unit_lots.filter(l => l.productId === productId.value)
    : store.unit_lots
);
...
function _recomputeAlerts() {
  ...
  const raw = StockAlert.buildFromLots(
    ...
    store.unit_lots,
    store.weight_lots,
  ).filter(a => a.productType === 'unit' && a.productId === pid);
...
function editUnitLot(id) {
  router.push({ name: 'inventory-unit-lots-list-edit', params: { id }, query: { productId: productId.value } });
}

function deleteUnitLot(id) {
  const lot = store.unit_lots.find(l => l.id === id);
  if (lot) store.deleteUnitLot(lot);
}

function navigateToCreate() {
  router.push({ name: 'inventory-unit-lots-list-new', query: { productId: productId.value } });
}
</script>

<template>
  <div class="list-overlay">
    <div v-if="!store.unit_lotsLoaded" class="spinner-wrapper">
      ...
      <div class="content-card">
        ...
        <div class="lots-table">
          <div class="table-header-labels">
            <span></span>
            <span>#ID</span>
            <span>{{ t('lots.form.quantity') }}</span>
            <span>{{ t('lots.form.entryDate') }}</span>
            <span>{{ t('lots.form.expiryDate') }}</span>
          ...
          <div v-for="lot in filteredLots" :key="lot.id" class="lot-row">
            ...
            <div class="row-action-cell">
              <span v-if="isExpired(lot.expiryDate)" class="expired-pill">{{ t('lots.status.expired') }}</span>
              <button
                  type="button"
                  class="btn-edit-square"
                  @click.stop="editUnitLot(lot.id)">
                <span class="material-icons">edit</span>
              </button>
            </div>

            <span class="id-cell">{{ t('lots.prefix') }}-{{ lot.id }}</span>
            <span class="qty-cell">{{ lot.quantity }} uds.</span>
            <span>{{ lot.entryDate ? fmtDate(lot.entryDate) : '-' }}</span>
            <span :class="{ 'warning-text': isExpired(lot.expiryDate) }">
              {{ lot.expiryDate ? fmtDate(lot.expiryDate) : '-' }}
            </span>
            <div class="qr-cell">
            ...
            <button
                ...
                @click.stop="deleteUnitLot(lot.id)"
...
.title-group p  { margin: 0; color: var(--color-text-muted); font-weight: 500; }
...
.table-header-labels {
  display: grid;
  grid-template-columns: 50px 1fr 1fr 1fr 1fr 1.5fr 50px;
  padding: 0 20px 15px;
...
.lot-row:hover { border-color: var(--color-primary); background: var(--color-theme-btn-active-bg); }
...
.warning-text { color: var(--color-danger) !important; font-weight: 700; }

.row-action-cell { position: relative; display: flex; align-items: center; justify-content: center; }
...
@media (max-width: 768px) {
  ...
  .table-header-labels,
  .lot-row {
    grid-template-columns: 44px 80px 70px 95px 95px 120px 44px;
    min-width: 550px;
  }
...
@media (max-width: 480px) {
  ...
  .lot-row > span:nth-child(4) { order: 5; flex: 1; font-size: 13px; }
  .lot-row > span:nth-child(5) { order: 6; flex: 1; font-size: 13px; }

  /* Row 4: QR code full width */
  .qr-cell { order: 7; flex: 0 0 100%; }
